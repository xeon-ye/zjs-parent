package com.zjs.commons.enums;

import com.zjs.commons.util.StringUtil;

/**
 * 统一定义缓存KEY
 * 备注： ①枚举name应遵守以VALUE、LIST、SET、ZSET、HASH等开头
 *      ②枚举code应尽量简写形式，以工程主名字为开头，例如：daxiong、daxiong_push
 *      ③ `:` 冒号分割的前后应该是一个名词词性，冒号前后是有上下级关系的，多个单词解释一个名词时，约定使用 `_`下划线分割
 * 
 * 举例： ①daxiong的用户缓存 daxiong:user:{userId}
 *      ②daxiong的某用户地址信息缓存 daxiong:user_address
 * 
 * Created by daxiong on 17/5/24.
 */
public enum CacheKeyEnum {
	 ...
	 * 登录TOKEN缓存key
	 */
	VALUE_LOGIN_TOKENS("daxiong:login_tokens:", TimeEnum.ONE_WEEK.sec()),

	/**
	 * 用户缓存
	 */
	VALUE_USERS("daxiong:user:profile:%s", TimeEnum.ONE_WEEK.sec());
